...
public class ThrowEx01 {
	...
	public static void main(String[] args) {
		...
		//예외 강제 발생시키기 throw
		/*
		 * 메서드나 생성자 실행도중에 throw 키워드를 만나면 즉시 throw로 생성한 예외가 발생합니다.
		 * 
...
//			System.out.println(e.getMessage()); //예외 발생시 전달할 예외 메세지를 얻어오는 기능
			e.printStackTrace(); //printStackTrace() 예외의 경로를 추적하는 메세지를 출력해줍니다.(예외 원인을 찾을 때 매번 사용됩니다.)
		}
	...
	public static int calc(int n) throws Exception {
		...
		if(n <= 0) {
			throw new Exception("0보다 큰값을 입력해야 합니다.");
		}
		int sum = 0;
		for(int i = 1; i <= n; i++) {
			sum += i;
